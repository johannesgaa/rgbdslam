    ...
    
{
    ...
    earliest_loop_closure_node_ = 0;//new_node->idKonstruktor_;
       ...
       ps->get<double>("min_rotation_degree") > 0.0)
    {
      ...
      if(mr.edge.id1 > 0 && mr.edge.id2 > 0) {//Found trafo
          std_msgs::Header convHeaderPrev;
          std_msgs::Header convHeaderNewNode;
          pcl_conversions::fromPCL(prev_frame->pc_col->header, convHeaderPrev);
          pcl_conversions::fromPCL(new_node->pc_col->header, convHeaderNewNode);

        ros::Time time1 = convHeaderPrev.stamp;
        ros::Time time2 = convHeaderNewNode.stamp;
    ...
    if (ps->get<bool>("concurrent_edge_construction")) 
    {
        ...
        std_msgs::Header convertedHeader;
        std_msgs::Header convertedHeaderBuffer;
        pcl_conversions::fromPCL(new_node->pc_col->header, convertedHeader);
        for (int i = 0; i < results.size(); i++) 
        {
            ...
            if (mr.edge.id1 >= 0 ) {
              ...
              pcl_conversions::fromPCL(graph_[mr.edge.id1]->pc_col->header,convertedHeaderBuffer);


              ros::Duration delta_time = convertedHeader.stamp - convertedHeaderBuffer.stamp;
        ...
        for (int id_of_id = (int) vertices_to_comp.size() - 1; id_of_id >= 0; id_of_id--) {
            ...
            if (mr.edge.id1 >= 0) {
              ...
              //ROS_INFO_STREAM("XX Information Matrix for Edge (" << mr.edge.id1 << "<->" << mr.edge.id2 << "\n" << mr.edge.informationMatrix);
                ...
                pcl_conversions::fromPCL(graph_[mr.edge.id1]->pc_col->header, convertedHeader);
              ros::Duration delta_time = convertedHeader.stamp - convertedHeader.stamp;
              if (isSmallTrafo(mr.edge.mean, delta_time.toSec()) &&
