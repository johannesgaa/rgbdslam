  ...
  initial_node_matches_(0)
{
  ...
  pcl_conversions::fromPCL(point_cloud->header, convertedHeader);
  base2points_ = tf::StampedTransform(tf::Transform::getIdentity(), convertedHeader.stamp,ParameterServer::instance()->get<std::string>("base_frame_name"), convertedHeader.frame_id);
  odom_transform_ = tf::StampedTransform(tf::Transform::getIdentity(), convertedHeader.stamp, "missing_odometry", convertedHeader.frame_id);
  ground_truth_transform_ = tf::StampedTransform(tf::Transform::getIdentity(), convertedHeader.stamp, ParameterServer::instance()->get<std::string>("ground_truth_frame_name"), ParameterServer::instance()->get<std::string>("base_frame_name"));
