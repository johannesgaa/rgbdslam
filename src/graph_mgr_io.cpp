...
void GraphManager::saveTrajectory(QString filebasename, bool with_ground_truth)
{
    ...
    for (graph_it it = graph_.begin(); it != graph_.end(); ++it){
        ...
        pcl_conversions::fromPCL( node->pc_col->header, convertedHeader);

        logTransform(et_out, world2base, convertedHeader.stamp.toSec());
...
void publishCloud(Node* node, ros::Time timestamp, ros::Publisher pub){
    ...
    ros::Time stamp =convertedHeader.stamp; //temp storage
    convertedHeader.stamp = timestamp; //to sync with tf
    pcl_conversions::toPCL(convertedHeader, node->pc_col->header);
    pub.publish(node->pc_col);
    convertedHeader.stamp = stamp; //restore original stamp (to minimize unexpected side-effects of this function)
    pcl_conversions::toPCL(convertedHeader, node->pc_col->header);
