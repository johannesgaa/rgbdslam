                                         ...
                                         const sensor_msgs::CameraInfoConstPtr& cam_info) 
{
  ScopedTimer s(__FUNCTION__);
  pointcloud_type* cloud (new pointcloud_type() );
  std_msgs::Header headerBuffer;
  headerBuffer.stamp     = depth_msg->header.stamp;
  headerBuffer.frame_id  = rgb_msg->header.frame_id;
  pcl_conversions::toPCL(headerBuffer, cloud->header);
  cloud->is_dense         = false; //single point of view, 2d rasterized NaN where no depth value was found
